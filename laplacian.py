...
def laplacian(input_dir, original_dir, out_dir):
    ...
    for img_name in os.listdir(input_dir):
        ...
        Ads = list(pyramid_gaussian(A, max_layer=4, multichannel=True))
        Ls = [Ads[i] - cv2.resize(Ads[i + 1], Ads[i].shape[1::-1]) for i in range(4)]
        Cus = list(accumulate([C] + Ls[::-1], lambda x, y: cv2.resize(x, y.shape[1::-1]) + y))
